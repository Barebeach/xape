...
waitForDependencies().then(() => {
  ...
  console.log('🚀 Page loaded - Loading XAPE orb (waiting for "initialize" command for visuals)')
  
  window.isAutoLoad = true
  window.isFirstInitialization = false
  document.body.classList.remove('extension-dormant')
  
  // Load XAPE orb/skillbar (voice agent only, NO heatmap)
  initializeExtension()
  
  // Skillbar's own voice recognition will handle "initialize" command
  console.log('🎤 XAPE orb loaded - say "initialize" to activate heatmap')
})

// Load all visual effects (heatmap, borders, animations)
// This will be called when user says "initialize" to the skillbar
function loadVisuals() {
  console.log('🎨 Loading all visual effects...')
...
// Make loadVisuals available globally so skillbar can call it
window.loadXapeVisuals = loadVisuals
